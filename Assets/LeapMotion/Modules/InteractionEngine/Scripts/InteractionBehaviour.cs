  ...
  public class InteractionBehaviour : MonoBehaviour, IInteractionBehaviour {
    ...
    public void BeginHover(List<InteractionController> controllers) {
      ...
      if (_hoveringControllers.Count == controllers.Count) {
        OnHoverBegin();
                ...
                GameController.btnAnimate = "";
                if (gameObject.name == "A1")
                {
                    GameController.btnAnimate = "A1";
                }
                else if(gameObject.name == "A2")
                {
                    GameController.btnAnimate = "A2";
                }
                else if (gameObject.name == "A3")
                {
                    GameController.btnAnimate = "A3";
                }
                else if (gameObject.name == "A4")
                {
                    GameController.btnAnimate = "A4";
                }
                else if (gameObject.name == "A5")
                {
                    GameController.btnAnimate = "A5";
                }
                else if (gameObject.name == "Next")
                {
                    GameController.btnAnimate = "Next";
                }
            }
    }
    ...
    public void EndHover(List<InteractionController> controllers) {
      ...
      if (_hoveringControllers.Count == 0) {
        OnHoverEnd();
                GameController.btnAnimate = "";
                GameController.targetTime = 3.0f;
            }
    }
    ...
    public void EndGrasp(List<InteractionController> controllers) {
      ...
      foreach (var controller in controllers) {
        ...
        GlobalFunction GFunction = Canvas.GetComponent<GlobalFunction>();
        if(GFunction.countShaking >= 30){
            // print("Letting Go Off The Object");
            

            // GFunction.DeleteTopWall();
        }
        GFunction.countShaking = 0;
    ...
    public void StayGrasped(List<InteractionController> controllers) {
      if (moveObjectWhenGrasped) {
        ...
        //     Player.Life--;
        // }
        // EnemyFireMove script = GetComponent<EnemyFireMove>();
       ...
       GameObject Canvas = GameObject.Find("Canvas");
        GlobalFunction GFunction = Canvas.GetComponent<GlobalFunction>();
        GFunction.countShaking++;
        // print("Object Being Hold Update Object");
      }
